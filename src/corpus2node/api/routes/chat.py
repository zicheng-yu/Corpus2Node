...
async def message(request: ChatRequest) -> ChatResponse:
    try:
        ...
        async with keyed_lock(f"chat:{request.session_id}"):
            chat = _load_chat(request.session_id)
            turn = await chat_agent.run_chat(
                _augment(request.message, request), ctx, model=model, history=chat.messages
            )
    ...
    async def event_source():
        ...
        async with keyed_lock(f"chat:{request.session_id}"):
            ...
            async for event in chat_agent.stream_chat_events(
                _augment(request.message, request), ctx, model=model, history=history
            ):
                if event.type == "token":
