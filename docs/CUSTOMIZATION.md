...
采用“**一个产品核心 + 个人账号基线 + 可选团队客户模式/适配器**”，不为每个客户复制仓库，也不维护长期客户分支。

2026-07-15 的产品决策再次收敛：当前默认交付为个人账号版，每个用户有一个不可切换、不可见的私有 `Organization`，它只承担资源和配额隔离；每个用户独立 BYOK。团队、成员、共享 Project 与公共修订保留在 `ACCOUNT_PRODUCT_MODE=teams`，以后面向明确客户单独设计，不进入当前线上 UI。PostgreSQL 保存身份、配额和资源索引，JSON artifact 继续是图谱与报告的事实来源；需要法规级物理隔离时，仍可按客户独立部署同一套代码。
...
| 部署覆盖层 | 环境与资源差异 | 域名、共享模型绑定、数据库、artifact 根目录、CPU/GPU、反向代理 | 托管环境或客户独立部署清单 |
...
- 每个用户固定到 `users.personal_organization_id`；`organizations` 在当前产品中不可见，只作为资源、套餐和用量边界。
- 每个 artifact 必须在 `resources` 登记 `organization_id`、`project_id`、owner 和路径；账号模式下未登记 artifact 不可见。
- session、图谱、科研报告、Chat、笔记、测试和提示偏好都只能由当前个人账号读取；跨账号读取统一返回 404。
- 模型凭据按 `artifacts/users/<user_id>/llm_settings.json` 保存，权限 600，服务器环境密钥不会自动注入新账号。
- `memberships`、共享 `projects`、活动流和公共修订仅在后续 `teams` 客户模式启用；现有 artifact 仍通过只登记、不移动文件的迁移工具导入。
- 物理隔离客户可使用独立 PostgreSQL、artifact 根目录和域名，但不维护另一套产品分支。
...
- 不复制多个仓库或维护长期客户分支。
- 不在当前个人版引入团队/成员 UI、项目级二次成员表、MFA、SSO、对象存储或多 worker；这些等待明确需求。
