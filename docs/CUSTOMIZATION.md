...
采用“**一个产品核心 + 托管多租户 + 可选客户配置/适配器**”，不为每个客户复制仓库，也不维护长期客户分支。

2026-07-14 的真实内测需求已经取代“暂不建设单实例多租户”的旧决定。当前默认交付为单实例托管多租户：`Organization` 是租户和计费边界，PostgreSQL 保存身份、权限、套餐、资源索引、修订和活动，JSON artifact 继续是图谱与报告的事实来源。需要法规级物理隔离时，仍可按客户独立部署同一套代码。
...
- `organizations` / `memberships` 建立租户与角色边界，`projects` 表示共享课题，现有 session 表示一次资料导入。
- 每个 artifact 必须在 `resources` 登记 `organization_id`、`project_id`、owner 和路径；账号模式下未登记 artifact 不可见。
- 图谱、科研报告和修订为团队共享；Chat、笔记、测试和提示偏好按组织与用户隔离；模型凭据平台私有。
- 跨租户读取统一返回 404；现有 artifact 通过只登记、不移动文件的迁移工具导入。
...
- 不复制多个仓库或维护长期客户分支。
- 不引入项目级二次成员表、MFA、SSO、BYOK、对象存储或多 worker；这些等待明确需求。
