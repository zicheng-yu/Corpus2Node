...
const STATUS_MAP: Record<SessionStatus, { cls: string; label: string }> = {
  ...
  ingested:    { cls: "chip chip-info",  label: "已解析" },
...
function matchFilter(status: SessionStatus, filter: FilterGroup): boolean {
  ...
  if (filter === "processing") return status === "ingesting" || status === "ingested" || status === "building_graph" || status === "merging_graph" || status === "uploaded" || status === "draft";
