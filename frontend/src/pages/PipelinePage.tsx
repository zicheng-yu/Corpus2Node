...
function PipelineCanvas({
  ...
  const docs = useMemo(
    () => Array.from({ length: 3 }, (_, i) => ({ x: 70 + i * 92, y: CY })),
    [],
  );
          ...
          if (event.type === "start") {
            ...
            if (node === "ingest") {
              ...
              if (data.concept_count != null)
                acc.concept = Number(data.concept_count);
              if (data.relation_count != null)
                acc.relation = Number(data.relation_count);
              push();
            } else if (node === "build") {
              acc.concept = Number(data.concept_count ?? acc.concept ?? 0);
              acc.relation = Number(data.relation_count ?? acc.relation ?? 0);
    ...
    <div className="pipeline-page">
      ...
      <div className="pipeline-viz">
        ...
        <div className="viz-counters">
          <span>
            片段 <b>{fmt(counts.chunk)}</b>
          </span>
          <span>
            概念 <b>{fmt(counts.concept)}</b>
          </span>
          <span>
            关系 <b>{fmt(counts.relation)}</b>
          </span>
          <span>
            社区 <b>{fmt(counts.cluster)}</b>
          </span>
        </div>
      ...
      {Object.keys(nodeMetrics).length > 0 && (
        <div className="run-metrics">
          <div className="run-metrics-head">
            ...
            {runTotals && (
              <span className="run-metrics-total">
                {fmtMs(runTotals.duration_ms)} · {runTotals.total_tokens} tokens
              </span>
            )}
          </div>
            ...
            {METRIC_NODES.filter((n) => nodeMetrics[n.key]).map((n) => {
              ...
              return (
                <div className="run-metric-row" key={n.key}>
                  ...
                  <span className="rm-tok">
                    {m.total_tokens > 0 ? `${m.total_tokens} tok` : "—"}
                  </span>
                  <span className="rm-rep">
                    {n.key === "critic" ? `修复 ${m.repair_count}` : ""}
                  </span>
                </div>
              );
            })}
        ...
        <div className="log-strip">
          <div className="log-row">
            ...
            <span className="log-ok">
              完成：{counts.concept} 概念 / {counts.relation} 关系 /{" "}
              {counts.cluster} 社区，正在进入工作区…
            </span>
          </div>
        </div>
      )}
      ...
      {runState === "failed" && (
        <div className="pipeline-error">{errorMsg ?? "处理失败，请重试。"}</div>
      )}
